...
def trace_filaments_single(
    ...
    coords, fom = apply_fom_filter(coords, fom, fom_min, fom_max)
    n = len(coords)
    progress_fn(f"[load] filtered picks: {n}")

    adj, graph_stats = build_candidate_graph(coords, candidate_k_std, max_neighbors)
    mean_d, std_d, cutoff, all_pairs, kept_pairs = graph_stats
    progress_fn(
        "[graph] candidate rule: "
        f"distance <= mean - k*std = {mean_d:.3f} - {candidate_k_std:.3f}*{std_d:.3f} = {cutoff:.3f}"
    )
    progress_fn(f"[graph] kept graph edges: {kept_pairs} / {all_pairs}")
    progress_fn(f"[trace] minimum filament length is fixed at {MIN_FILAMENT_POINTS} points")
    ...
    filaments = []
    growth_frames = []
    if growth_movie_out is not None:
    ...
    for seed in seed_order:
        ...
        chain = grow_chain(seed, adj, coords, used, max_line_rms)
        if growth_movie_out is not None and len(chain) >= 1:
    ...
    overlay_path = None if no_save_overlay else (out_prefix.parent / f"{out_prefix.name}_overlay.png")
    plot_filaments(
        mrc_path,
        overlay_path,
        coords,
    ...
    if save_npz:
        ...
        np.savez_compressed(
            ...
            max_line_rms=np.array([max_line_rms]),
        )

    used_count = sum(len(c) for c in filaments)
    progress_fn(f"[done] filaments kept: {len(filaments)}")
    progress_fn(f"[done] picks assigned to filaments: {used_count} / {n}")
    if overlay_path is not None:
        progress_fn(f"[done] saved overlay: {overlay_path.resolve()}")
    else:
        progress_fn("[done] overlay PNG disabled (--no-save-overlay)")
    if growth_movie_out is not None:
    ...
    if save_npz:
        progress_fn(f"[done] saved trace summary: {npz_path.resolve()}")
    else:
        progress_fn("[done] trace summary NPZ disabled (use --save-npz to enable)")

    return {
        "filaments": filaments,
        "filtered_pick_count": n,
        "assigned_pick_count": used_count,
        "out_prefix": out_prefix,
        "endpoints_star_path": endpoints_star_path,
        "overlay_path": overlay_path,
        "npz_path": npz_path if save_npz else None,
        "candidate_rule_mean": mean_d,
        "candidate_rule_std": std_d,
        "candidate_rule_cutoff": cutoff,
